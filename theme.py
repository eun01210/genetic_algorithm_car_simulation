...
def manywife(genepool, ranking):
    ...
    for i in range(1, 2):
        ...
        sd.append(tempsd[0])
        '''
        tempmf = genepool[ranking.index(0)]
        print(len(tempmf))
        for i in range(50):
            del(tempmf[len(tempmf)-1])
...
class lidars(pygame.sprite.Sprite):
    ...
    def move(self, x, y, angle, image):
        ...
        if self.s == "m":
            self.rect.center = (x - math.sin(math.radians(angle))*50, y + math.cos(math.radians(angle))*50) # 위치 객체의 위치 설정
        elif self.s == "l":
            self.rect.center = (x - math.sin(math.radians(angle))*50, y + math.cos(math.radians(angle))*50)
        elif self.s == "r":
            self.rect.center = (x - math.sin(math.radians(angle))*50, y + math.cos(math.radians(angle))*50)
...
def inits(n, cros, extr):
    ...
    while gen:
        if genetic:
            ...
            genepool.append([])
        lifetime.append(0)
        car = cars(car_image, random.randrange(1050, 1051), 300)
        ...
        try:
            if i.go:
                if genepool[con][fme] != -999:
                    ...
                    if genepool[con][fme] == 999: i.angle = 180
                    i.angle += random.randrange(-10, 11)
                ...
                while genepool[con][len(genepool[con])] == -999:
                    genepool[con].pop(len(genepool[con]))
        except:
            if i.go:
                i.angle += random.randrange(-10, 11)
            ...
            if i.go or (nowleft[con]+nowmid[con]+nowright[con] == 0) or (nowleft[con]+nowmid[con]+nowright[con] == 3):
                mini = 0
                maxi = 0
                if nowleft[con]: 
                    maxi += 5
