...
def lastche(bgxy):
    if 340<=bgxy[len(bgxy)-1][1]<=360 and 999<=bgxy[len(bgxy)-1][0]<=1112:
        return True
    else:
        return False
...
class cars(pygame.sprite.Sprite):
    ...
    def move(self, con):
        ...
        if lineconf(self.x, self.y, checkp[sect][0], checkp[sect][1], checkp[sect][2], checkp[sect][3]):
            ...
            if len(templist) <= 8: return self.x, self.y, self.angle, self.go
...
if len(bestestcar) >= 350:
	f = open('./dist/result-elet-ext.txt', 'a')
	f.write('%s/%s\n' %(str(len(bestestcar)), str(time.time()-start_time)))
	f.close()
else:
    g = open('./dist/error-elet-ext.txt', 'a')
    g.write('1\n')
    g.close()
